...
import os
from Utils import LassoLoss
from Utils import RigdeLoss
    ...
    for col in cols:
        ...
        try:
            ...
            try:
                ...
                datatypes.append(tf.constant(0, dtype=tf.int32))
            except tf.errors.InvalidArgumentError:
                datatypes.append(tf.constant(0, dtype=tf.int32))
    return datatypes
...
def main():
    ...
    model.summary()
    model.compile(optimizer=Adam(), loss=RigdeLoss(weights=model.trainable_weights, lambda_=1e-4), metrics=[mse_metric()])
    model.fit(train_gen, validation_data=val_gen, epochs=2000, callbacks=[rlp_obj,tb])
